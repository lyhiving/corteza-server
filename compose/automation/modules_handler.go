...
func ModulesHandler(reg modulesHandlerRegistry, ns moduleNamespaceService, mod moduleService) *modulesHandler {
	h := &modulesHandler{
		reg: reg,
		ns:  ns,
		mod: mod,
...
func (h modulesHandler) resolveNamespace(ctx context.Context, id *uint64, handle string, res *types.Namespace) (err error) {
	if *id == 0 {
		if len(handle) > 0 {
